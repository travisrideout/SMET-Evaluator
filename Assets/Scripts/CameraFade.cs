...
public class CameraFade : MonoBehaviour {
        ...
        if (Time.time > instance.m_FadeDelay) {
            ...
            if (instance.m_CurrentScreenOverlayColor != instance.m_TargetScreenOverlayColor) {
                ...
                if (Mathf.Abs(instance.m_CurrentScreenOverlayColor.a - instance.m_TargetScreenOverlayColor.a) < Mathf.Abs(instance.m_DeltaColor.a) * Time.deltaTime) {
                    ...
                    if (instance.loadSceneOnFinish) {
                        loadLevelOnComplete(instance.sceneIndex);
                        instance.loadSceneOnFinish = false;
                    }
    ...
    /// <summary>
    /// Starts the fade from color newScreenOverlayColor. If isFadeIn, start fully opaque, else start transparent, after a delay. Load scene (sceneIndexNumber) when finished.
    /// </summary>
    /// <param name='newScreenOverlayColor'>
    /// New screen overlay color.
    /// </param>
    ...
    /// Fade delay.
    /// </param>
    /// <param name='sceneIndexNumber'>
    /// Scene index number to load on finish.
    /// </param>
    public static void StartAlphaFade(Color newScreenOverlayColor, bool isFadeIn, float fadeDuration, float fadeDelay, int sceneIndexNumber) {
        if (fadeDuration <= 0.0f) {
            SetScreenOverlayColor(newScreenOverlayColor);
        } else {
            instance.sceneIndex = sceneIndexNumber;
            instance.loadSceneOnFinish = true;
            instance.m_FadeDelay = Time.time + fadeDelay;
